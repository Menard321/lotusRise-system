...
@section('title', 'Create Project — LotusRise Global')
@section('page_title', 'Add New Portfolio Project')
...
<div class="max-w-2xl space-y-8">
    ...
    <div class="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
        <h2 class="text-lg font-bold text-[#0D0C0A] mb-6">New Portfolio Entry</h2>
        ...
        <form action="{{ route('admin.portfolio.store') }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <select name="category" required class="block w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#C5A85A] text-gray-700">
                    ...
                    <option value="Vendor Solutions">Vendor Solutions</option>
            ...
            <div>
                ...
                <textarea name="description" required rows="3"
                    class="block w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#C5A85A]">{{ old('description') }}</textarea>
            </div>

            <div>
                <label class="block text-xs font-semibold uppercase tracking-wider text-gray-600 mb-1">Case Study / Impact Story</label>
                <textarea name="case_study" rows="4" placeholder="Optional: Describe measurable impact or key results..."
                    class="block w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-[#C5A85A]">{{ old('case_study') }}</textarea>
            </div>
